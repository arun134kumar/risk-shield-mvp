...
export default function StatementView({ statementId }) {
  const { caseData, authToken } = useAnalysis();
  const statementObj = useMemo(() => caseData?.statements?.find(s => s.id === statementId), [caseData, statementId]);
  const data = statementObj?.analysisResult || {};
  const loading = false;
  ...
  useEffect(() => {
      // Event listener for demo mode upload was removed in favor of Tabs
  }, []);
  ...
  if (!statementObj || data.error) return <div style={{textAlign: 'center', marginTop: '4rem'}}>No data found for this statement.</div>;
